...
func (h *transactionHandler) CreateTransaction(c *gin.Context) {
	...
	if err != nil {
		...
		response := helper.APIResponse("Failed to create transactionsss baris 69", http.StatusUnprocessableEntity, "error", errorMessage)
	...
	if err != nil {
		response := helper.APIResponse("Failed to create transaction baris 82", http.StatusBadRequest, "error", nil)
		c.JSON(http.StatusBadRequest, response)
