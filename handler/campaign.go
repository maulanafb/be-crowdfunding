...
func (h *campaignHandler) UploadImage(c *gin.Context) {
	...
	if err != nil {
		errors := helper.FormatValidationError(err)
		errorMessage := gin.H{"errors": errors}
		response := helper.APIResponse("Failed to uplod campaign", http.StatusUnprocessableEntity, "error", errorMessage)
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	response := helper.APIResponse("success upload campaign images", http.StatusOK, "success", data)
	c.JSON(http.StatusOK, response)
